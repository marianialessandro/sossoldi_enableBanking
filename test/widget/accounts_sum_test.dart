...
class _TestCurrencyState extends CurrencyState {
  @override
  Currency build() => const Currency(id: 1, symbol: '€', code: 'EUR', name: 'Euro', mainCurrency: true);
}

void main() {
  testWidgets('Account summary renders its supplied total and selected currency', (WidgetTester tester) async {
    SharedPreferences.setMockInitialValues({'visibility_amount': true});
    final preferences = await SharedPreferences.getInstance();
    const account = BankAccount(id: 99, name: 'Synthetic account', symbol: 'account_balance', color: 0, startingValue: 305.89, total: 232.75, active: true, countNetWorth: true, mainAccount: false, order: 2);

    await tester.pumpWidget(
      MaterialApp(
        home: Material(
          child: ProviderScope(
            overrides: [sharedPrefProvider.overrideWithValue(preferences), currencyStateProvider.overrideWith(_TestCurrencyState.new)],

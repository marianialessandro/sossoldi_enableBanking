...
class BankMoney {
  ...
  static const scales = <String, int>{'EUR': 2, 'USD': 2, 'GBP': 2, 'CHF': 2, 'CAD': 2, 'AUD': 2, 'NZD': 2, 'SEK': 2, 'NOK': 2, 'DKK': 2, 'PLN': 2, 'CZK': 2, 'HUF': 2, 'RON': 2, 'BGN': 2, 'ISK': 0, 'JPY': 0, 'KRW': 0, 'CLP': 0, 'BHD': 3, 'KWD': 3, 'OMR': 3, 'JOD': 3, 'TND': 3};
  ...
  factory BankMoney.parse(String amount, String currency) {
    ...
    if (fraction.length > scale && fraction.substring(scale).contains(RegExp('[1-9]'))) {
    ...
    return BankMoney._(currency, match[1] == '-' ? -magnitude : magnitude, scale);
  }
  ...
  num get projection {
    ...
    if (BankMoney.parse(value.toStringAsFixed(scale), currency).minorUnits != minorUnits) {

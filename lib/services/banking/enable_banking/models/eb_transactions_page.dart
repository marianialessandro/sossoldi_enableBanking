...
class EbTransactionsPage {
  ...
  const EbTransactionsPage({this.transactions = const [], this.continuationKey, this.rejectedRecords = 0, this.serverTime});

  static EbTransactionsPage fromJson(Map<String, dynamic> json, {bool collectRejectedRecords = false}) {
    ...
    for (final record in raw) {
      try {
        transactions.add(EbTransaction.fromJson(record as Map<String, dynamic>));
      } on FormatException {
    ...
    return EbTransactionsPage(transactions: transactions, continuationKey: key == null || key.trim().isEmpty ? null : key, rejectedRecords: rejected, serverTime: json['_server_date'] == null ? null : DateTime.parse(json['_server_date'] as String));
  }
}

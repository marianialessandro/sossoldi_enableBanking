...
BankConnectionRepository bankConnectionRepository(Ref ref) =>
    BankConnectionRepository(database: ref.watch(databaseProvider));

enum BankReconciliationFailure {
...
class BankConnectionRepository {
  ...
  BankConnectionRepository.withDatabase(Database database)
  ...
  Future<BankConnection?> findByPendingAuthorizationId(String id) async {
    final db = await _database;
    final rows = await db.query(
      bankConnectionTable,
      columns: BankConnectionFields.allFields,
      where: '${BankConnectionFields.pendingAuthorizationId} = ?',
      whereArgs: [id],
      limit: 1,
    );
    return rows.isEmpty ? null : BankConnection.fromJson(rows.single);
  ...
  Future<List<BankConnection>> selectAwaitingImport() async {
    ...
    final rows = await db.query(
      bankConnectionTable,
      columns: BankConnectionFields.allFields,
      where: '${BankConnectionFields.pendingSessionId} IS NOT NULL',
    );
    return rows.map(BankConnection.fromJson).toList(growable: false);
  }

  Future<List<BankConnection>> selectActive({
    ...
    return db.transaction((txn) async {
      await txn.update(
        bankConnectionTable,
        {
          BankConnectionFields.status: BankConnectionStatus.expired.code,
  ...
  Future<BankConnection> stageSession({
    ...
    required String psuType,
    int? reconnectConnectionId,
  }) async {
    if (psuType != 'personal') {
      throw const BankReconciliationException(
        BankReconciliationFailure.invalidConnectionState,
        'Only personal bank connections are supported',
      );
    }
    final db = await _database;
    return db.transaction((txn) async {
      ...
      if (reconnectConnectionId == null) {
        final connection = BankConnection(
          ...
          pendingAuthorizationId: authorizationId,
          pendingValidUntil: validUntil,
          status: BankConnectionStatus.awaitingImport,
          psuType: psuType,
        );
        final values = connection.toJson(clock: now)
          ..remove(BankConnectionFields.id);
      ...
      final rows = await txn.query(
        ...
        columns: BankConnectionFields.allFields,
        where: '${BankConnectionFields.id} = ?',
        whereArgs: [reconnectConnectionId],
        limit: 1,
      );
      if (rows.isEmpty) {
        throw const BankReconciliationException(
          BankReconciliationFailure.invalidConnectionState,
          'Reconnect target does not exist',
        );
      }
      final existing = BankConnection.fromJson(rows.single);
      if (existing.applicationId != applicationId ||
          existing.aspspName != aspspName ||
          existing.aspspCountry != aspspCountry ||
          !{
        ...
        throw const BankReconciliationException(
          BankReconciliationFailure.invalidConnectionState,
          'Reconnect session does not match the existing connection',
        );
      }
      final staged = existing.copy(
        pendingSessionId: sessionId,
        pendingAuthorizationId: authorizationId,
        pendingValidUntil: validUntil,
        psuType: psuType,
  ...
  Future<BankConnection> activateStagedSession(
    int connectionId,
    List<BankAccountLink> links,
  ) async {
    final db = await _database;
    return db.transaction((txn) async {
      final connectionRows = await txn.query(
        bankConnectionTable,
        columns: BankConnectionFields.allFields,
        where: '${BankConnectionFields.id} = ?',
      ...
      if (connection.pendingSessionId == null ||
          connection.pendingValidUntil == null) {
        throw const BankReconciliationException(
          BankReconciliationFailure.invalidConnectionState,
          'Connection has no staged session',
        );
      }
      if (links.isEmpty) {
        throw const BankReconciliationException(
          BankReconciliationFailure.missingLocalAccount,
          'At least one account must be selected before activation',
        );
      }
      ...
      for (final link in links) {
        final hashes = link.identificationHashes
            .map((hash) => hash.trim())
            .where((hash) => hash.isNotEmpty)
            .toSet();
        if (link.uid.trim().isEmpty || hashes.isEmpty) {
          throw const BankReconciliationException(
            BankReconciliationFailure.missingStableIdentity,
            'A selected account has no stable identification hash',
          );
        }
        if (hashes.any(remoteHashes.contains)) {
          throw const BankReconciliationException(
            BankReconciliationFailure.identityCollision,
            'Two remote accounts expose the same identification hash',
          );
        }
      ...
      final identityRows = await txn.query(
        ...
        where: '${BankAccountIdentityFields.connectionId} = ?',
        whereArgs: [connectionId],
      );
      final identityOwners = <String, int>{
        for (final row in identityRows)
          row[BankAccountIdentityFields.identificationHash] as String:
              row[BankAccountIdentityFields.bankAccountId] as int,
      };
      ...
      for (final link in normalized) {
        ...
        if (matchingIds.length > 1) {
          throw const BankReconciliationException(
            BankReconciliationFailure.identityCollision,
            'Stable hashes resolve to different local accounts',
          );
        }
        ...
        if (matchingIds.length == 1) {
          ...
          final selectedRows = await txn.query(
            bankAccountTable,
            columns: [BankAccountFields.ebConnectionId],
            where: '${BankAccountFields.id} = ?',
            whereArgs: [existingId],
            limit: 1,
          );
          if (selectedRows.isEmpty ||
              (selectedRows.single[BankAccountFields.ebConnectionId] != null &&
                  selectedRows.single[BankAccountFields.ebConnectionId] !=
                      connectionId)) {
            throw const BankReconciliationException(
              BankReconciliationFailure.missingLocalAccount,
              'Selected local account belongs to another connection',
            );
          }
          ...
          final count =
              Sqflite.firstIntValue(
                await txn.rawQuery('SELECT COUNT(*) FROM `$bankAccountTable`'),
              ) ??
              0;
          final values = draft.copy(order: count).toJson()
            ..remove(BankAccountFields.id);
          accountId = await txn.insert(bankAccountTable, values);
        } else {
          throw const BankReconciliationException(
            BankReconciliationFailure.missingLocalAccount,
            'A new remote account needs a local account definition',
          );
        }
        if (!selectedAccountIds.add(accountId)) {
          throw const BankReconciliationException(
            BankReconciliationFailure.identityCollision,
            'One local account was selected more than once',
          );
        }
        ...
        if (changed != 1) {
          throw const BankReconciliationException(
            BankReconciliationFailure.missingLocalAccount,
            'Selected local account does not exist',
          );
        }
        await txn.delete(
          bankAccountIdentityTable,
        ...
        for (final hash in sortedHashes) {
          await txn.insert(bankAccountIdentityTable, {
            BankAccountIdentityFields.connectionId: connectionId,
            BankAccountIdentityFields.bankAccountId: accountId,
            BankAccountIdentityFields.identificationHash: hash,
          });
        }
      }
      ...
      final activated = connection.copy(
        sessionId: connection.pendingSessionId,
        pendingSessionId: null,
        pendingAuthorizationId: null,
        validUntil: connection.pendingValidUntil,
        pendingValidUntil: null,
        status: BankConnectionStatus.active,
      );
      await txn.update(
        bankConnectionTable,
        activated.toJson(update: true),
        where: '${BankConnectionFields.id} = ?',
        whereArgs: [connectionId],
      );
      return activated;
  ...
  Future<void> markStatus(int id, BankConnectionStatus status) async {
    ...
    final changed = await db.update(
      bankConnectionTable,
      {
        ...
        BankConnectionFields.updatedAt: DateTime.now()
            .toUtc()
            .toIso8601String(),
      },
      where: '${BankConnectionFields.id} = ?',
  ...
  Future<void> discardStagedSession(int id) async {
    final db = await _database;
    final changed = await db.update(
      bankConnectionTable,
      {
        BankConnectionFields.pendingSessionId: null,
        BankConnectionFields.pendingAuthorizationId: null,
        BankConnectionFields.pendingValidUntil: null,
        BankConnectionFields.updatedAt: DateTime.now()
            .toUtc()
            .toIso8601String(),
      },
      where:
          ...
          '${BankConnectionFields.sessionId} IS NOT NULL',
      whereArgs: [id],
    );
    if (changed != 1) {
      throw StateError('Bank connection $id has no previous session');
    }
  }

  Future<void> failStagedSession(int id, BankConnectionStatus status) async {
    final db = await _database;
    final changed = await db.update(
      bankConnectionTable,
      {
  ...
  Future<void> markOtherApplicationsReauthRequired(String applicationId) async {
    final db = await _database;
    await db.update(
      bankConnectionTable,
      {
        BankConnectionFields.status: BankConnectionStatus.reauthRequired.code,
        BankConnectionFields.updatedAt: DateTime.now()
            .toUtc()
            .toIso8601String(),
      },
      ...
      whereArgs: [
        applicationId,
        BankConnectionStatus.revoked.code,
        BankConnectionStatus.disconnected.code,
        BankConnectionStatus.revocationPending.code,
      ],
    );
  }

  Future<void> markAllReauthRequired() async {
    final db = await _database;
    await db.update(
      bankConnectionTable,
      {
      ...
      where: '${BankConnectionFields.status} NOT IN (?, ?, ?)',
      whereArgs: [
        BankConnectionStatus.revoked.code,
        BankConnectionStatus.disconnected.code,
        BankConnectionStatus.revocationPending.code,
  ...
  Future<void> finalizeRemoteRevocation(int id) async =>
      _finalizeDisconnect(id, BankConnectionStatus.revoked);

  Future<void> disconnectLocally(int id) async =>
      _finalizeDisconnect(id, BankConnectionStatus.disconnected);
  ...
  Future<void> _finalizeDisconnect(int id, BankConnectionStatus status) async {
    ...
    await db.transaction((txn) async {
      ...
      final changed = await txn.update(
        ...
        {
          BankConnectionFields.status: status.code,
          BankConnectionFields.sessionId: null,
          BankConnectionFields.pendingSessionId: null,
          BankConnectionFields.pendingAuthorizationId: null,
          BankConnectionFields.pendingValidUntil: null,
          BankConnectionFields.updatedAt: DateTime.now()

...
class AddBankConsentLifecycle extends Migration {
  AddBankConsentLifecycle()
    : super(
        version: 8,
        description: 'Add bank consent lifecycle and stable account identity',
      );
  ...
  Future<void> up(Database db) async {
    await db.execute('''
      CREATE TABLE `$bankConnectionTable` (
        ...
        `${BankConnectionFields.sessionId}` TEXT,
        `${BankConnectionFields.pendingSessionId}` TEXT,
    ...
    await db.execute('''
      ...
      ON `$bankConnectionTable` (`${BankConnectionFields.sessionId}`)
      WHERE `${BankConnectionFields.sessionId}` IS NOT NULL
    ''');
    await db.execute('''
